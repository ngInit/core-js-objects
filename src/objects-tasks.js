...
function mergeObjects(objects) {
  return objects.reduce((accumulator, item) => {
    Object.entries(item).forEach(([key, value]) => {
      accumulator[key] = value + (accumulator[key] || 0);
    });
    return accumulator;
  }, {});
}
...
function removeProperties(obj, keys) {
  const objCopy = { ...obj };
  keys.filter((key) => delete objCopy[key]);
  return objCopy;
}
...
function compareObjects(obj1, obj2) {
  return Object.values(obj1).every(
    (key, index) => key === Object.values(obj2)[index]
  );
}
...
function sellTickets(queue) {
  ...
  for (let index = 0; index < queue.length; index += 1) {
    if (queue[index] === cost) {
      total += cost;
    } else if (queue[index] - cost > total) {
      return false;
    } else {
      total += queue[index] - cost;
    }
  }
  return true;
}
...
function sortCitiesArray(arr) {
  return arr.sort((firstCity, secondCity) => {
    if (firstCity.country === secondCity.country) {
      return firstCity.city > secondCity.city ? 1 : -1;
    }
    return firstCity.country > secondCity.country ? 1 : -1;
  });
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((object) => {
    ...
    const newGroup = resultMap.get(key) || [];
    newGroup.push(value);
    resultMap.set(key, newGroup);
  });
  return resultMap;
}
